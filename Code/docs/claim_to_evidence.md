...
| C39 | Modality ablation | Fusion vs EMG-only vs audio-only | `figures/modality_comparison.png`; `metrics.json` → `modality_contrast` | same | **blocked-data** | Ran; matched windows, folds, seeds and budget. But the microphone channel is duplicated across participants (`audio.md` §1.1, Q13), so the audio conditions were never held out on that channel. Reported as run; bounds a duplicated channel, does not measure a microphone. |
| C40 | Modality ablation | Audio benefit **excluding chewing** | `metrics.json` → `chewing_contrast` | same | **blocked-data** | Was to be the make-or-break analysis (`Temp.md` item E): if the gain vanished without chewing, the contribution would be "cheap eating rejection". It can no longer be made *or* broken from this collection — the channel it tests is duplicated across participants, so neither the hypothesis nor its negation is evaluable. Needs a new acquisition (`audio.md` §6). |
